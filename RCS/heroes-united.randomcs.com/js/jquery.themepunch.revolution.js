		...
		function initSlider(container,opt) {
						...
						if (container.data('aimg')!=undefined) {
							if ((container.data('aie8')=="enabled" && isIE(8)) || (container.data('amobile')=="enabled" && is_mobile()))
								container.html('<img class="tp-slider-alternative-image" src="../../heroes united/js/'+container.data("aimg")+'">');
						}
		...
		var prepareSlides = function(container,opt) {
			...
			container.find('>ul:first >li >img').each(function(j) {
				...
				if (isIE(8)) {
					...
					pari.find('.tp-bgimg').append('<img class="ieeightfallbackimage defaultimg" src="../../heroes united/js/'+src+'" style="width:100%">');
				}
						...
						for (var j=0;j<opt.slots;j++) {
							...
							for (var i=0;i<opt.slots;i++) 	{
								...
								if (isIE(8)) {
									...
									sh.find('.slot ').last().find('.slotslide').append('<img src="../../heroes united/js/'+imgsrc+'">');
						 ...
						 if (vorh == "horizontal") {
							...
							for (var i=0;i<opt.slots;i++) {
									...
									if (scalestart!=undefined && rotatestart!=undefined)
										punchgs.TweenLite.set(sh.find('.slot').last(),{rotationZ:rotatestart});
									if (isIE(8)) {
									   sh.find('.slot ').last().find('.slotslide').append('<img class="ieeightfallbackimage" src="../../heroes united/js/'+imgsrc+'" style="width:100%;height:auto">');
									   ieimgposition(sh,opt);
								}
							}
						} else {
									...
									if (nextcaption.hasClass("tp-videolayer") || nextcaption.find('iframe').length>0 || nextcaption.find('video').length>0 ) {
										...
										if ((videomp!=undefined || videowebm!=undefined) && nextcaption.find('video').length==0) {
											...
											/*if (nextcaption.data('videoposter')!=undefined)
												apptxt = apptxt + 'poster="../../heroes united/js/'+nextcaption.data('videoposter')+'">';
												apptxt = apptxt + '<source src="../../heroes united/js/'+videomp+'" type="video/mp4" />';
												apptxt = apptxt + '<source src="../../heroes united/js/'+videowebm+'" type="video/webm" />';
												apptxt = apptxt + '<source src="../../heroes united/js/'+videoogv+'" type="video/ogg" />';
												apptxt = apptxt + '</video>';
											nextcaption.append(apptxt);*/
											
											if (nextcaption.data('videoposter')!=undefined)
												if (nextcaption.data('videoposter') != undefined) apptxt = apptxt + 'poster="'+nextcaption.data('videoposter')+'">';
												if (videowebm!=undefined && get_browser().toLowerCase()=="firefox") apptxt = apptxt + '<source src="../../heroes united/js/'+videowebm+'" type="video/webm" />';
												if (videomp!=undefined) apptxt = apptxt + '<source src="../../heroes united/js/'+videomp+'" type="video/mp4" />';
												if (videoogv!=undefined) apptxt = apptxt + '<source src="../../heroes united/js/'+videoogv+'" type="video/ogg" />';
												apptxt = apptxt + '</video>';
											nextcaption.append(apptxt);
		...
		var nextli = container.find('>ul:first-child >li:eq('+opt.next+')'),
			...
			nextsh.find('.defaultimg').each(function() {
				...
				if (nextsh.find('.kenburnimg').length==0)
					nextsh.append('<div class="kenburnimg" style="position:absolute;z-index:1;width:100%;height:100%;top:0px;left:0px;"><img src="../../heroes united/js/'+defimg.attr('src')+'" style="-webkit-touch-callout: none;-webkit-user-select: none;-khtml-user-select: none;-moz-user-select: none;-ms-user-select: none;user-select: none;position:absolute;width:'+imgobj.w+'%;height:'+imgobj.h+'%;"></div>');
				else {
		...
		**************************************************************************/
		...
		var createThumbs = function(container,opt) {
			...
			container.find('>ul:first >li').each(function(i) {
							...
							bullets.append('<div class="bullet thumb" style="background-color:'+bgcolor+';position:relative;width:'+opt.thumbWidth+'px;height:'+opt.thumbHeight+'px;background-image:url('../../heroes united/js/+src+') !important;background-size:cover;background-position:center center;"></div>');
							var bullet= bullets.find('.bullet:first');
				});
